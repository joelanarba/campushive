      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          <div className="bg-paper-raised dark:bg-ink-raised rounded-2xl border border-line dark:border-line-dark shadow-sm overflow-hidden">
            ...
            {loading ? (
              <div className="p-10 text-center text-ink-soft dark:text-paper/70">Loading bookings...</div>
            ) : bookings.length === 0 ? (
              ...
              <div className="divide-y divide-line dark:divide-line-dark">
                {bookings.map(booking => {
                  ...
                  return (
                    <div key={booking.id} className="p-6 flex flex-col md:flex-row gap-4 justify-between">
                      <div>
                        <div className="flex items-center gap-3 mb-2">
                          <h3 className="font-bold text-lg">{booking.service?.title || booking.service_name_snapshot}</h3>
                        ...
                        <p className="text-sm font-medium text-ink-soft mb-1">
                          Student: {booking.user?.full_name} ({booking.user?.email})
                        </p>
                      ...
                      <div className="flex flex-col gap-2 shrink-0">
                        {booking.status === 'pending' && (
                          <>
                            <button onClick={() => handleUpdateBooking(booking.id, 'confirmed')} className="px-3 py-1.5 bg-green-600 text-white text-sm font-medium rounded hover:bg-green-700 transition-colors">Accept Booking</button>
                            <button onClick={() => handleUpdateBooking(booking.id, 'cancelled')} className="px-3 py-1.5 bg-red-100 text-red-700 text-sm font-medium rounded hover:bg-red-200 transition-colors">Decline</button>
                          </>
                        )}
                        {booking.status === 'confirmed' && (
                          <button onClick={() => handleUpdateBooking(booking.id, 'completed')} className="px-3 py-1.5 bg-blue-600 text-white text-sm font-medium rounded hover:bg-blue-700 transition-colors">Mark Completed</button>
                        )}
                      </div>
                    </div>
                  );
        ...
        <div className="lg:col-span-1 space-y-8">
          <div className="bg-paper-raised dark:bg-ink-raised rounded-2xl border border-line dark:border-line-dark shadow-sm overflow-hidden">
            ...
            {loading ? (
              <div className="p-6 text-center text-ink-soft dark:text-paper/70">Loading services...</div>
            ) : services.length === 0 ? (
                ...
                {services.map((svc) => (
                  <div key={svc.id} className="p-6 flex flex-col gap-4">
                    ...
                    <div className="flex items-center gap-2 pt-3 border-t border-line dark:border-line-dark">
                      <Button to={`/dashboard/entrepreneur/services/${svc.id}/availability`} variant="secondary" className="px-3 py-2 h-auto text-xs" title="Manage Schedule">
                        <Clock size={14} className="mr-1" /> Schedule
                      </Button>
                      <Button to={`/dashboard/entrepreneur/services/${svc.id}/edit`} variant="secondary" className="px-3 py-2 h-auto text-xs">
                        <Edit2 size={14} />
                      </Button>
                      <button 
